...

{
    ...
    while (index <= mrna_string.length())
    {
        ...
        index = aug_pos + 3;                   //update index after aug
        string result = "M";
        while (1)
        {
            string t = "";
            ...
            index = index + 3; //update index
            if (index > mrna_string.length())
                break;
            for (int i = 0; i < mycodons.size(); i++) //search codon
            {
                if (mycodons[i][0] == t)
                {
                    result += mycodons[i][1];
            ...
            if (t == "UAG" || t == "UGA" || t == "UAA")
            {
                ...
                mrna_string = mrna_string.substr(index); //update string
                break;
            }
        }
    }
